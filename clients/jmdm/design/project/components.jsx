    ...
    <>
      ...
      <div className={"fbag " + (open ? "is-open" : "")}>
        <button
          ...
          aria-expanded={open}>
          <img src="assets/jmdm-lemon.svg" alt="" className="fbag__lemon__img" draggable="false" />
...
// ---- image lightbox (zoom modal), shared by detail pages
function Lightbox({ photo, onClose }) {
  ...
  return (
    <div className="lightbox" onClick={onClose}>
      <button className="lightbox__close" onClick={onClose} aria-label="close">× close</button>
      <div className="lightbox__inner" onClick={(e) => e.stopPropagation()}>
        <Placeholder kind={photo.kind} label={photo.label} num={photo.num} />
      </div>
    </div>);
...
// ---- foot
function Foot({ go }) {
  const { adminMode, setAdminMode, contact, updateContact } = window.useAdmin();
  return (
    <footer className="foot">
      <div className="foot__col foot__col--contact" data-comment-anchor="901711509c-div-206-7">
        <h6>contact</h6>
        {adminMode ?
        <>
            <window.InlineText admin className="inline-edit--foot" value={contact.email} onChange={(v) => updateContact({ email: v })} placeholder="email" />
            <window.InlineText admin className="inline-edit--foot" value={contact.instagram} onChange={(v) => updateContact({ instagram: v })} placeholder="@handle" />
          </> :
        <>
            <a href={"mailto:" + contact.email}>{contact.email}</a>
            <a href={"https://instagram.com/" + contact.instagram.replace(/^@/, "")} target="_blank" rel="noreferrer">instagram · {contact.instagram}</a>
          </>
        }
      </div>
...
Object.assign(window, { Wordmark, LemonSigil, Masthead, Placeholder, RowPreview, Foot, pickKind, Lightbox });